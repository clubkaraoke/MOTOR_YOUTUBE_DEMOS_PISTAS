...
var M={mode:"karaoke",timeline:null,cdg:null,decoder:null,processed:0,busy:false,lastProject:"",clearMode:"delayed",previewScale:2};
...
function opts(){var x={clear_mode:M.clearMode};try{x.lines_per_screen=Number(S.cfg.linesPerPage||6);x.font_size=Number(S.cfg.fontSize||18);x.stroke_width=Number(S.cfg.strokeWidth||1);x.lyric_y_offset=Number(S.cfg.lyricYOffset||0)}catch(_){}return x}
...
function inject(){
  ...
  ".v2body{padding:0 9px 9px;display:flex;flex-direction:column;gap:8px;min-height:0;flex:1}.v2options{display:grid;grid-template-columns:1fr 1fr;gap:7px}.v2opt{display:flex;flex-direction:column;gap:3px;font:9px monospace;color:#93a1b4}.v2opt select{background:#171c25;color:#fff;border:1px solid #343d4d;border-radius:5px;padding:6px;font:700 10px Arial}.v2screen{aspect-ratio:300/216;background:#111427;border:1px solid #343b47;display:flex;align-items:center;justify-content:center;overflow:hidden}.v2screen canvas{width:100%;height:100%;image-rendering:auto}"+
  ...
  '<div class="v2body"><div class="v2options"><label class="v2opt">Filas Nomad<select id="v2clear"><option value="eager">EAGER · reemplaza antes</option><option value="delayed" selected>DELAYED · espera más</option><option value="page">PAGE · página completa</option></select></label><label class="v2opt">Resolución preview<select id="v2scale"><option value="1">1× · 300×216</option><option value="2" selected>2× · 600×432</option><option value="4">4× · 1200×864</option></select></label></div><div class="v2screen"><canvas id="v2canvas" width="600" height="432"></canvas></div><div id="v2panel"></div>'+
...
async function autoRefreshWhenReady(){
  for(var i=0;i<240;i++){
    if(editorReady()){
      try{var cm=String((S.doc.cdg_settings||{}).clear_mode||"delayed").toLowerCase();if(["eager","delayed","page"].includes(cm))M.clearMode=cm;var cs=el("v2clear");if(cs)cs.value=M.clearMode}catch(_){}
      await refresh(false);
      return;
    }
...
async function refresh(force){
  ...
  try{var p=project();var d=await request("/api/v2/jobs/"+encodeURIComponent(jid)+"/timeline",{project:p,options:opts()});M.timeline=d.timeline;M.cdg=null;resetDecoder();status("Timeline V2 lista · filas NOMAD "+String(M.clearMode).toUpperCase()+" · START/END conservados.","ok");paintPanel();paint()}
