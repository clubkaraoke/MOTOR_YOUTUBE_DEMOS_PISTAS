...
def security_headers(response):
    ...
    response.headers.setdefault('Content-Security-Policy',"default-src 'self'; img-src 'self' data:; media-src 'self' blob:; connect-src 'self' https://content.dropboxapi.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' data: https://fonts.gstatic.com; script-src 'self' 'unsafe-inline'; base-uri 'self'; frame-ancestors 'self'")
...
def create_job_upload():
    ...
    try:
        ...
        if not letra: raise ValueError('Falta la letra de Moisés.')
        artist,title=master_identity(source_inst_name); voice_duration=float(request.form.get('voice_duration') or 0)
