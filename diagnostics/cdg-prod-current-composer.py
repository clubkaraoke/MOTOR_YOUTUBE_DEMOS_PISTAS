...
class KaraokeComposer:
    ...
    def __init__(
        ...
        if len(self.config.singers) >= 4:
            d, f, m, duo = self.config.singers[:4]
            # Paleta optimizada para cuatro roles con el mismo texto base blanco/negro.
            # 4/5 = base; XOR 2=default, XOR 8=mujer, XOR 10=hombre, XOR 12=duo.
            self.color_table = [
                self.config.background,
                self.config.border or self.UNUSED_COLOR,
                self.UNUSED_COLOR,
                self.UNUSED_COLOR,
                d.inactive_fill, d.inactive_stroke,
                d.active_fill, d.active_stroke,
                duo.active_fill, duo.active_stroke,
                d.inactive_fill, d.inactive_stroke,
                f.active_fill, f.active_stroke,
                m.active_fill, m.active_stroke,
            ]
        else:
            self.color_table = [
                self.config.background,
            ...
            for singer in self.config.singers:
                self.color_table.extend([singer.inactive_fill,singer.inactive_stroke,singer.active_fill,singer.active_stroke])
            self.color_table = list(pad(self.color_table,16,padvalue=self.UNUSED_COLOR))
        ...
        for ci, lyric in enumerate(self.config.lyrics):
            ...
            for li, (line, singer, line_image, line_mask) in enumerate(
                ...
                # Get enough sync points/modes for this line's syllables
                line_sync = lyric.sync[sync_i : sync_i + len(line)]
                line_modes = lyric.syllable_modes[sync_i : sync_i + len(line)] if lyric.syllable_modes else [0] * len(line)
                sync_i += len(line)
                if line_sync:
                ...
                syllables: list[SyllableInfo] = []
                for si, (mask, syllable, (start, end), mode) in enumerate(
                    zip(
                        line_mask,
                        line,
            ...
            if line_draw_info.text.strip():
                state.draw_queue.extend(
                    line_image_to_packets(
                        ...
                        fill=(10 if line_draw_info.singer == 2 else 4),
                        stroke=(11 if line_draw_info.singer == 2 else 5),
                        background=self.BACKGROUND,
                    )
                )
            else:
    ...
    def _compose_instrumental(
        ...
        try:
            ...
            text_images = render_lines(
                ...
                stroke_width=(1 if instrumental.stroke is not None else 0),
                stroke_type=self.config.stroke_type,
            )
    ...
    def _compose_intro(self):
        ...
        # Layout protegido de la portada DJGABO. El título es prioritario y
        # el artista tiene su propia caja; ninguno puede invadir el logo.
        def fit_box(text, max_size, min_size, box, max_lines):
            x1, y1, x2, y2 = box
            width, height = x2-x1, y2-y1
            chosen = None
            for size in range(max_size, min_size-1, -1):
                ft = ImageFont.truetype(self.config.font, size)
                wrapped = get_wrapped_text(text, font=ft, width=width).split("\n")
                if len(wrapped) > max_lines:
                    continue
                imgs = list(render_lines(wrapped, font=ft))
                total_h = sum(max(1, im.height) for im in imgs)
                if total_h <= height and all(im.width <= width for im in imgs):
                    chosen = (ft, imgs)
                    break
            if chosen is None:
                ft = ImageFont.truetype(self.config.font, min_size)
                wrapped = get_wrapped_text(text, font=ft, width=width).split("\n")[:max_lines]
                chosen = (ft, list(render_lines(wrapped, font=ft)))
            return chosen

        title_box = (8, 16, 292, 62)
        artist_box = (18, 84, 282, 120)
        title_font, title_imgs = fit_box(self.config.title, 26, 16, title_box, 2)
        artist_font, artist_imgs = fit_box(self.config.artist, 20, 14, artist_box, 2)

        def paste_box(imgs, box, palette_index):
            x1, y1, x2, y2 = box
            heights = [max(1, im.height) for im in imgs]
            total_h = sum(heights)
            y = y1 + max(0, (y2-y1-total_h)//2)
            for im, h in zip(imgs, heights):
                xx = x1 + max(0, (x2-x1-im.width)//2)
                colored = im.point(lambda v: v and palette_index, "P")
                background_image.paste(colored, (xx, y), mask=im.point(lambda v: v and 255, "1"))
                y += h

        paste_box(title_imgs, title_box, 2)
        paste_box(artist_imgs, artist_box, 3)
    ...
    def _compose_outro(self, end: int):
        ...
        smallfont = ImageFont.truetype(self.config.font, 18)
        MAX_HEIGHT = 200
        ...
        text_image = text_image.crop(bbox) if bbox else Image.new("P", (1, 1), 0)
