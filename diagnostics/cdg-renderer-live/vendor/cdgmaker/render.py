...
def render_text(
    ...
    """
    # Glifo especial para el contador instrumental. No depende de la fuente:
    # cada § se dibuja como un círculo sólido de 18 px. Esto evita que Impact
    # o cualquier otra familia muestre rectángulos por falta del glifo Unicode.
    if "§" in text:
        circle_d = 18
        circle_adv = 22
        ascent, descent = font.getmetrics()
        stroke_pad = max(1, 2 * stroke_width)
        widths = [(circle_adv if c == "§" else font.getlength(c)) for c in text]
        image_width = max(1, int(sum(widths) + stroke_pad + font.size * 2))
        image_height = ascent + descent + stroke_pad
        image = Image.new("P", (image_width, image_height), 0)
        draw = ImageDraw.Draw(image); draw.fontmode = "1"
        x = font.size + stroke_width
        cy = stroke_width + ascent // 2 + 1
        for c, adv in zip(text, widths):
            if c == "§":
                r = circle_d // 2
                box = (int(x), cy-r, int(x)+circle_d, cy+r)
                if stroke_fill is not None:
                    draw.ellipse(box, fill=fill, outline=stroke_fill, width=max(1, stroke_width or 2))
                else:
                    draw.ellipse(box, fill=fill)
            elif c.strip():
                draw.text((int(x), stroke_width), c, fill, font)
            x += adv
        return image

    # El rótulo INSTRUMENTAL necesita borde negro aunque la letra normal use stroke 0.
    if text.strip().upper() == "INSTRUMENTAL" and stroke_width == 0:
        stroke_width = 1
